...
import { websiteTypeSchema } from "@/schemas/website_type.schema";
import { createWebsiteType } from "@/services/website_type.service";
import { WebsiteTypeAttribute } from '@/types/website_type'
type FormData = z.infer<typeof websiteTypeSchema>;
...
export default function WebsiteTypeAddForm() {
  ...
  const {
    ...
    resolver: zodResolver(websiteTypeSchema),
  });
  ...
  const onNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Step 2: Replace & with "and" manually
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (response?.status === true) { SwalSuccess("Website type added successfully."); handleBack(); }
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-12">
            <h1 className="text-xl font-medium text-gray-800 ml-2">Add Website Type</h1>
          </div>
        </div>
      </header>
